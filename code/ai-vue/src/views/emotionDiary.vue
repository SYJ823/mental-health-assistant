            ...
            <div class="diary-card">
                ...
                <div class="emotion-grid">
                    <div v-for="emotion in emotionOptions" :key="emotion.name" class="emotion-card" :class="{'selected': emotion.name === diaryForm.dominantEmotion}" @click="selectEmotion(emotion.name)">
                        <el-image :src="emotion.url" alt="情绪" class="emotion-icon" style="width: 50px; height: 50px;"></el-image>
...
// 情绪评分
const emotionStatus = ['绝望崩溃', '消沉抑郁', '焦虑烦躁', '低落不悦', '平静淡然', '轻松惬意', '愉悦愉悦', '欢欣满足', '兴奋欣喜', '极致幸福']
//情绪选项
...
const submitForm = () => {
    // 验证表单数据
    if (!diaryForm.moodScore || !diaryForm.dominantEmotion || !diaryForm.emotionTriggers || !diaryForm.diaryContent || !diaryForm.sleepQuality || !diaryForm.stressLevel) {
