  ...
  <div class="consultation-container">
    ...
    <div class="chat-main">
      <div class="chat-header">
        ...
        <el-button circle @click="createFrontendSession" title="新建会话">
      ...
      <div class="chat-messages">
        ...
        >
          <div class="message-avatar">
            <el-image
              :src="msg.senderType === 2 ? robotIcon : userIcon"
              alt="头像"
              style="width: 18px; height: 18px"
            />
          </div>
      ...
      <div class="chat-input">
        <div class="input-container">
          ...
          <div class="input-footer">
            <span>按Enter发送，按Enter+Shift换行</span>
...
const robotIcon = new URL("@/assets/images/robot-fill.png", import.meta.url)
  .href;
...
const sendMessage = () => {
  ...
  if (!message || message.length > 500) {
    ElMessage.warning(
      !message ? "请输入您的消息" : "消息长度不能超过500个字符",
    );
    return;
  }
...
const startNewSession = (message) => {
  const sessionParams = {
    ...
    sessionTitle: `心理AI助手-${new Date().toLocaleString()}`,
  };
  console.log("创建新会话参数:", sessionParams);
  //调用后台接口创建新会话
  startSession(sessionParams).then((res) => {
    console.log("创建新会话返回数据", res);
    //将后端返回的数据转为前端会话格式
    const sessionData = {
...
const startAIresponse = (sessionId, userMessage) => {
  //防止重复调用流式会话效果
  if (isAiTyping.value) {
    ElMessage.warning("AI正在输入中,稍后再发送");
    return;
  }
  ...
  fetchEventSource("/api/psychological-chat/stream", {
    ...
    onmessage: (event) => {
      ...
      if(ok && payload.data && payload.data.content){
        aiMessage.content += payload.data.content; //实时更新ai消息内容
      }else if(!ok){
        console.log('错误回复:',ok,payload.message);
